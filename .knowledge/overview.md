...
## Bảng so sánh kỹ thuật (từ source code)

| Project | Search Algorithm | Consolidation | Memory Evolution | Lifecycle Hooks | Offline? | DB |
|---|---|---|---|---|---|---|
| **agentmemory** | BM25+Vector+Graph RRF(K=60) | LLM per concept, max 8 obs | versioning: parentId+supersedes[] | Hook + REST + MCP | Có (SQLite) | SQLite (iii-engine) |
| **claude-mem** | SQLite FTS5 + Chroma vector | Session summary (LLM, end only) | Không có | Hooks native (SessionStart/PostToolUse/SessionEnd) | Có | SQLite + Chroma |
| **Mem0** | Vector + Keyword + Entity + Temporal | Không có | Không có | Không có | Có (OSS mode) | Pluggable (30+) |
| **Honcho** | Semantic via Dialectic tools | Dreamer (surprisal-based, async) | Reasoning trees (premise→conclusion) | Không native | Không (PG required) | PostgreSQL + pgvector |
| **RetainDB** | Vector search + pack/delta | Không có | Không có | Không native | Không (API dep) | API-driven |
| **Hindsight** | 4-parallel: semantic+BM25+graph+temporal → cross-encoder rerank | Không có | Không có | Không native | Không (LLM dep) | PostgreSQL + pgvector |
| **OpenViking** | HNSW ANN (C++ core), L0/L1/L2 tiered | Không có | Không có | Session end extraction | Có | Multi-tier (C++) |
| **Hermes** | SQLite FTS5 (built-in) + provider-specific | Không native | Không có | Plugin-based | Có (built-in only) | SQLite + provider |
| **superpowers** | Không có (skill-based) | Không có | Không có | SessionStart injection | Có | Không |
| **ICM** | Hybrid: FTS5+vector+graph expansion | Auto ≥10 entries (no versioning) | Không (xóa + tạo lại) | Không (hoàn toàn thủ công) | Có (Rust + SQLite) | SQLite (libsql) |
| **Understand-Anything** | Fuse.js fuzzy + cosine similarity | Không (one-time analysis) | Không | Không (static analysis tool) | Có (JSON file) | JSON (git-committable) |
| **A-mem** | Vector (ChromaDB) + 1-hop link expansion | LLM bidirectional (per add) | Không (destructive rebuild) | Không | Không (research code) | ChromaDB |
| **Zep** | Vector + Graph traversal + Temporal filter | LLM auto-extract (async) | valid_at/invalid_at per fact | Không native | Không (hosted-only SaaS) | Temporal graph (Graphiti) |
| **MemOS** | BM25 + Vector + Graph + Internet, RRF+MMR | LLM iterative (loop until "can answer") | version + history[] + status | Không native | Có (Neo4j + Qdrant local) | Neo4j + Qdrant + Milvus |
...
Sau khi đọc source code tất cả project, có 5 gap không ai giải quyết đầy đủ:

### Gap 1: Hybrid Storage (Centralized Data + Local Config)
- **Claude-mem** dùng global store nhưng thiếu local config file làm anchor → path-drift nếu git root thay đổi
- **Không ai** implement pattern: UUID config trong project + data tập trung ở global server
- Đây là gap lớn nhất, ảnh hưởng trực tiếp đến team collaboration và project mobility

### Gap 2: Smart Macro-Graph (tránh Hairball)
- **Không ai** chỉ vẽ liên kết ở mức Module/Entity/Decision/Constraint
- Mọi graph implementation đều có xu hướng vẽ micro-links (file → function → call)
- Kết quả: Hairball Problem — graph có hàng nghìn node nhiễu, mất giá trị với agent

### Gap 3: Global/Local Memory Boundary thực sự
- **Không ai** implement tách biệt rõ global knowledge vs local project knowledge
- Mọi system đều dùng một kho duy nhất, scope bằng `user_id`/`project_id` nhưng không có boundary kiến trúc

### Gap 4: Merge Wizard có chủ đích
- **Không ai** có pipeline wizard để user chọn tiêu chí merge (chủ đề/đối tượng/khái niệm/quyết định)
- Các system hiện tại hoặc merge tự động (không kiểm soát) hoặc không merge

### Gap 5: Situational Query — Reasoning-Ready Context
- **Honcho** gần nhất với reasoning context nhưng không lưu "tại sao" (rationale, trade-offs, constraints)
- Không ai trả về context theo format: `bối cảnh → ràng buộc → trade-offs → quyết định`
...
### `superpowers`
- Có hỗ trợ nạp Skill động qua hệ thống plugin của nền tảng (Claude Code, Cursor, Gemini CLI, v.v.).
- Có hỗ trợ cưỡng chế quy trình có kỷ luật thông qua các hướng dẫn logic nghiêm khắc (brainstorming, planning, TDD, SDD).
- Có hỗ trợ sinh và lưu trữ specs thiết kế tĩnh trực tiếp trong dự án git.
- Không thấy hỗ trợ database bộ nhớ động để lưu logs/facts/concepts tự động.
- Không thấy hỗ trợ tìm kiếm chéo hay đồng bộ hóa tự động ngoài git repository.

### `ICM`
- Có hỗ trợ dual-layer memory: Memory (decay-based) và Memoir/Concept (permanent knowledge graph).
- Có hỗ trợ hybrid search: FTS5 BM25 + vector similarity + graph expansion qua related_ids.
- Có hỗ trợ `icm_wake_up` token-budgeted context pack cho session start.
- Có hỗ trợ auto-dedup (similarity > 0.85), auto-link (related_ids), auto-consolidate (≥10 entries).
- Có hỗ trợ `icm_learn` scan project directory → Memoir knowledge graph.
- Có hỗ trợ Transcript (verbatim session replay) và Feedback (correction learning).
- Có hỗ trợ local-first hoàn toàn: Rust + SQLite + FastEmbed, không cần cloud.
- Không thấy hỗ trợ lifecycle hooks tự động — agent phải chủ động gọi mọi tool thủ công.
- Không thấy hỗ trợ memory versioning (không có parentId/supersedes khi consolidate).
- Không thấy hỗ trợ global/local project boundary thực sự ở local storage.
- `icm_learn` tạo micro-nodes (file lẻ, CI workflow) → dễ rơi vào Hairball Problem.

### `Understand-Anything`
- Có hỗ trợ hybrid analysis: tree-sitter (deterministic structure) + LLM (semantic meaning).
- Có hỗ trợ 21 node types + 35 edge types — rich schema cho code, business domain, và knowledge.
- Có hỗ trợ fingerprinting-based incremental updates: chỉ re-analyze files thay đổi (SHA-256 + signature).
- Có hỗ trợ 4-tier schema validation pipeline: sanitize → normalize (alias) → auto-fix → referential integrity.
- Có hỗ trợ committable JSON graph: team chia sẻ qua git, không cần mỗi người tự analyze.
- Có hỗ trợ domain node hierarchy: `domain → flow → step` tách bạch business logic khỏi code structure.
- Có hỗ trợ 7-phase multi-agent pipeline với parallel batching (5 concurrent, 20-30 files/batch).
- Có hỗ trợ persona-adaptive UI (non-technical / junior / experienced dev).
- Không phải memory system: không có decay, versioning, session continuity, hay lifecycle hooks.
- Không scale tốt với graph > 5,000 nodes (React Flow performance, O(n) search).

### `A-mem`
- Có hỗ trợ **bidirectional memory evolution**: memory mới không chỉ link đến memories cũ mà còn có thể update ngược metadata (context + tags) của neighbors qua LLM decision.
- Có hỗ trợ semantic triple: keywords + context + tags được extract bằng LLM cho mọi memory.
- Có hỗ trợ neighbor link expansion trong search: kết quả vector search được mở rộng bằng linked neighbors.
- Không thấy hỗ trợ lifecycle hooks, không production-ready (research code).
- Không thấy hỗ trợ versioning hay audit trail thực sự; consolidation destructive (rebuild toàn bộ index).
- LLM call mỗi add_note() — không scale được ở production.

### `Zep`
- Có hỗ trợ **Temporal Knowledge Graph**: mỗi fact có `valid_at` + `invalid_at` — track khi nào fact đúng, khi nào hết hiệu lực. Unique trong nhóm.
- Có hỗ trợ auto entity extraction từ raw conversations (Person, Location, Event, Preference, v.v.).
- Có hỗ trợ Context Templates: declarative format cho context assembly, tách format khỏi retrieval logic.
- Có hỗ trợ contextualized document chunking: inject document-level context vào mỗi chunk.
- Có hỗ trợ pluggable reranking: cross_encoder / RRF / MMR.
- Có hỗ trợ sub-200ms retrieval latency (design goal).
- Không thấy hỗ trợ self-hosted: hosted-only SaaS, offline không được.
- Không thấy hỗ trợ global/local project boundary hay lifecycle hooks tự động.

### `MemOS`
- Có hỗ trợ Memory OS-level orchestration với 4 layers: Parametric / Activation / Plaintext / Preference.
- Có hỗ trợ MemScheduler (Redis Streams): async task processing, không block user response.
- Có hỗ trợ full audit trail: version + history[] + status lifecycle (activated/resolving/archived/deleted).
- Có hỗ trợ iterative search: LLM judge "can I answer?" → generate new retrieval phrases → loop.
- Có hỗ trợ Activation Memory (KV-cache injection): 35%+ token savings.
- Có hỗ trợ Feedback → Skill crystallization loop.
- Không thấy hỗ trợ lifecycle hooks tự động; không native global/local project boundary.

### `cognee`
- Có hỗ trợ Deterministic UUID5 identity-based deduplication.
- Có hỗ trợ Ontology-grounded entity extraction (OWL, fuzzy matching 80%).
- Có hỗ trợ 15+ search strategies với auto-routing (TRIPLET, GRAPH_COMPLETION, TEMPORAL, v.v.).
- Có hỗ trợ 3-tier memory: session cache + permanent graph + typed entries (TraceEntry, SkillRunEntry).
- Có hỗ trợ MCP native (stdio/SSE/HTTP) — direct Claude integration.
- Có hỗ trợ provenance bắt buộc trên mọi DataPoint: source_pipeline, source_task, source_content_hash.
- Không thấy hỗ trợ lifecycle hooks tự động; sync Tier1→Tier2 cần gọi improve() thủ công.

### `ByteRover CLI`
- Có hỗ trợ Hierarchical Context Tree (DAG): d0-d3 condensation với auto-summary regeneration.
- Có hỗ trợ Runtime Signals Sidecar: tách ranking signals khỏi version-controlled content.
- Có hỗ trợ Lane Budgeting: token-aware manifest selection cho 3 lanes (summaries/contexts/stubs).
- Có hỗ trợ 4-tier query execution: cache → BM25 → pre-fetch+LLM → full agentic.
- Có hỗ trợ Dream background system: consolidate/prune/archive với HITL review.
- Có hỗ trợ Staleness detection qua children_hash trong summary frontmatter.
- Không thấy hỗ trợ global/local project boundary; git dependency cho version control.

### `Letta-code`
- Có hỗ trợ agent self-editing memory blocks (writable system prompt segments) — agent tự modify behavior.
- Có hỗ trợ Git-backed MemFS: full version control cho memory, git log/revert/push.
- Có hỗ trợ 11-event hook system: PreToolUse (BLOCK), SessionStart/End, Stop, PermissionRequest.
- Có hỗ trợ Layered skill discovery: Project > Agent > Global > Bundled với override semantics.
- Có hỗ trợ Dual-mode backend: API (remote) hoặc Local (no login) với BackendCapabilities flags.
- Có hỗ trợ Stateful subagents với riêng memory block access và tool whitelist.
- Không thấy hỗ trợ structured memory schema; memory blocks là free-form markdown.
...
### `superpowers`
Pipeline học của nó dựa trên tính kỷ luật cưỡng chế quy trình (Process/Workflow learning loop):
- **Brainstorm**: Khai thác ý tưởng của người dùng thông qua Socratic dialogue trước khi làm bất kỳ hành động code nào.
- **Spec Documentation**: Chuyển đổi và lưu trữ thiết kế đã được duyệt thành tài liệu tĩnh (`spec.md`) lưu tại dự án.
- **Planning**: Phân rã spec thành các task siêu nhỏ (2-5 phút thực thi) đi kèm mã nguồn mẫu và cách kiểm thử cụ thể.
- **Verification (TDD)**: Thực thi vòng lặp Red-Green-Refactor, xóa sạch code viết trước test để ép tính kỷ luật.
